...
const { sendRiskOtpEmail, sendPasswordResetEmail, isMailConfigured } = require('../utils/mailer');
...
const getFrontendBaseUrl = () => process.env.FRONTEND_URL || 'http://localhost:4200';

const clearResetPasswordState = (user) => {
  ...
  try {
    ...
    if (user.isLocked) {
      ...
      return res.status(423).json({
        success: false,
        code: 'ACCOUNT_LOCKED',
        message: `Compte temporairement bloqué. Réessayez dans ${Math.ceil(retryAfterSeconds / 60)} minute(s).`,
        retryAfterSeconds
      });
    }
    ...
    if (!isPasswordValid) {
      ...
      if (user.lockUntil && user.failedLoginAttempts >= maxAttempts) {
...
exports.forgotPassword = async (req, res) => {
  try {
    ...
    const expiresInSeconds = Number(process.env.AUTH_RESET_PASSWORD_TTL_SECONDS || 3600);
    const rawToken = crypto.randomBytes(32).toString('hex');
    const tokenHash = sha256(rawToken);

    user.resetPasswordTokenHash = tokenHash;
    user.resetPasswordExpiresAt = new Date(Date.now() + expiresInSeconds * 1000);
    await user.save({ validateBeforeSave: false });

    const resetUrl = `${getFrontendBaseUrl()}/login?mode=reset&token=${rawToken}`;
    let delivery = 'unconfigured';
    ...
    return res.status(200).json({
      ...
      data: {
        delivery,
        ...(delivery !== 'email' && includeDevToken ? { resetToken: rawToken, resetUrl } : {})
      }
    });
  } catch (error) {
